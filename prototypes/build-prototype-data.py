...
# per-day MAE, for the "which day" surfaces
day_mae = {d: {m: sum(abs(days[d]["o"][h]-days[d][m][h]) for h in range(24))/24 for m in MODELS} for d in allday}
...
payload = {
    ...
    "dayMae": {d: {m: round(v[m], 2) for m in MODELS} for d, v in day_mae.items()},
    "labels": {"c": "Chronos-2", "a": "AR-168", "n": "Day-lag naive"},
}
